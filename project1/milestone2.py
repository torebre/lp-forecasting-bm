...
cancelled_by_year_and_week = cancelled_bookings.filter(items=["Date (Year and Week Number", "IsCancelled"]).groupby(
    "Date (Year and Week Number")

# cancelled_by_year_and_week.size()
cancelled_by_year_and_week.value_counts()

decomposition = seasonal_decompose(cancelled_by_year_and_week, model = "additive")
